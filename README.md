...
### Easiest: the local studio (`studio.html`)

A local control panel that lists every slot with a copy-ready prompt and a drop zone for the
winner. It's **not part of the published site** (gitignored) and must be opened over a local
server so it can write files:

```bash
python3 -m http.server          # from the repo root
# open http://localhost:8000/studio.html  (use Chrome or Edge for one-click save)
```

In the studio: click **Connect repo folder** once (pick the `natural-explorers` folder) →
for each slot, **Copy prompt** and paste into Gemini/ChatGPT → **drop or paste** the winner →
**Save**, which writes it straight into `assets/illustrations/` with the correct filename.
Then tell Claude **"publish art"** (or run `bin/publish-art.sh` / `/publish-art`) to push it live.

> Safari/Firefox lack the file-write API, so Save downloads the correctly-named PNG instead —
> move it into `assets/illustrations/` yourself, then publish.
...
2. **Generate** — open `IMAGE-PROMPTS.md`, prepend the **Global Style Preamble** to a slot's
   prompt, paste into Gemini/ChatGPT at the listed aspect ratio.
3. **Pick** a favorite (keep it anywhere, e.g. `~/Downloads/`).
4. **Publish one file** in a single command:
   ```bash
   bin/add-art.sh hero-waterfall.png ~/Downloads/my-favorite.png
   ```
   (or `/add-art hero-waterfall.png ~/Downloads/my-favorite.png` inside Claude Code). It copies
   the file in with the correct name, commits, and pushes — Pages republishes in ~1 min.
